...
package com.third.SwipeListView2;
...
public class SwipeListViewTouchListener implements View.OnTouchListener {
    ...
    private int swipeMode = SwipeListViewWrap.SWIPE_MODE_BOTH;
    ...
    // Fixed properties
    private SwipeListViewWrap swipeListView;
    ...
    private int swipeCurrentAction = SwipeListViewWrap.SWIPE_ACTION_NONE;

    private int swipeActionLeft = SwipeListViewWrap.SWIPE_ACTION_REVEAL;
    private int swipeActionRight = SwipeListViewWrap.SWIPE_ACTION_REVEAL;
     ...
     */
    public SwipeListViewTouchListener(SwipeListViewWrap swipeListView, int swipeFrontView, int swipeBackView) {
    ...
    protected boolean isSwipeEnabled() {
        return swipeMode != SwipeListViewWrap.SWIPE_MODE_NONE;
    }
    ...
    private void swapChoiceState(int position) {
        ...
        if (lastCount == 0 && count == 1) {
            ...
            closeOpenedItems();
            setActionsTo(SwipeListViewWrap.SWIPE_ACTION_CHOICE);
        }
    ...
    protected int getCountSelected() {
        ...
        if(SwipeListViewWrap.DEBUG){
            Log.d(SwipeListViewWrap.TAG, "selected: " + count);
        }
        return count;
    }
    ...
    private void generateAnimate(final View view, final boolean swap, final boolean swapRight, final int position) {
        if(SwipeListViewWrap.DEBUG){
            Log.d(SwipeListViewWrap.TAG, "swap: " + swap + " - swapRight: " + swapRight + " - position: " + position);
        }
        if (swipeCurrentAction == SwipeListViewWrap.SWIPE_ACTION_REVEAL) {
            generateRevealAnimate(view, swap, swapRight, position);
        }
        if (swipeCurrentAction == SwipeListViewWrap.SWIPE_ACTION_DISMISS) {
            generateDismissAnimate(parentView, swap, swapRight, position);
        }
        if (swipeCurrentAction == SwipeListViewWrap.SWIPE_ACTION_CHOICE) {
    ...
    public boolean onTouch(View view, MotionEvent motionEvent) {
        ...
        switch (MotionEventCompat.getActionMasked(motionEvent)) {
            case MotionEvent.ACTION_DOWN: {
                ...
                swipeCurrentAction = SwipeListViewWrap.SWIPE_ACTION_NONE;
            ...
            case MotionEvent.ACTION_UP: {
                ...
                if (!opened.get(downPosition)) {
                    if (swipeMode == SwipeListViewWrap.SWIPE_MODE_LEFT && velocityTracker.getXVelocity() > 0) {
                        velocityX = 0;
                    }
                    if (swipeMode == SwipeListViewWrap.SWIPE_MODE_RIGHT && velocityTracker.getXVelocity() < 0) {
                        velocityX = 0;
                    }
                }
                ...
                if (minFlingVelocity <= velocityX && velocityX <= maxFlingVelocity && velocityY * 2 < velocityX) {
                    ...
                    if(SwipeListViewWrap.DEBUG){
                        Log.d(SwipeListViewWrap.TAG, "swapRight: " + swapRight + " - swipingRight: " + swipingRight);
                    }
...
//                }
            ...
            case MotionEvent.ACTION_MOVE: {
                ...
                if (swipeMode == SwipeListViewWrap.SWIPE_MODE_NONE) {
                    deltaMode = 0;
                } else if (swipeMode != SwipeListViewWrap.SWIPE_MODE_BOTH) {
                    if (opened.get(downPosition)) {
                        if (swipeMode == SwipeListViewWrap.SWIPE_MODE_LEFT && deltaX < 0) {
                            deltaMode = 0;
                        } else if (swipeMode == SwipeListViewWrap.SWIPE_MODE_RIGHT && deltaX > 0) {
                            deltaMode = 0;
                        }
                    } else {
                        if (swipeMode == SwipeListViewWrap.SWIPE_MODE_LEFT && deltaX > 0) {
                ...
                if (deltaMode > slop && swipeCurrentAction == SwipeListViewWrap.SWIPE_ACTION_NONE && velocityY < velocityX) {
                    swiping = true;
                    swipingRight = (deltaX > 0);
                    if(SwipeListViewWrap.DEBUG){
                        Log.d(SwipeListViewWrap.TAG, "deltaX: " + deltaX + " - swipingRight: " + swipingRight);
                    }
                    if (opened.get(downPosition)) {
                        swipeListView.onStartClose(downPosition, swipingRight);
                        swipeCurrentAction = SwipeListViewWrap.SWIPE_ACTION_REVEAL;
                    } else {
                        if (swipingRight && swipeActionRight == SwipeListViewWrap.SWIPE_ACTION_DISMISS) {
                            swipeCurrentAction = SwipeListViewWrap.SWIPE_ACTION_DISMISS;
                        } else if (!swipingRight && swipeActionLeft == SwipeListViewWrap.SWIPE_ACTION_DISMISS) {
                            swipeCurrentAction = SwipeListViewWrap.SWIPE_ACTION_DISMISS;
                        } else if (swipingRight && swipeActionRight == SwipeListViewWrap.SWIPE_ACTION_CHOICE) {
                            swipeCurrentAction = SwipeListViewWrap.SWIPE_ACTION_CHOICE;
                        } else if (!swipingRight && swipeActionLeft == SwipeListViewWrap.SWIPE_ACTION_CHOICE) {
                            swipeCurrentAction = SwipeListViewWrap.SWIPE_ACTION_CHOICE;
                        } else {
                            swipeCurrentAction = SwipeListViewWrap.SWIPE_ACTION_REVEAL;
                        }
    ...
    public void move(float deltaX) {
        ...
        if (posX > 0 && !swipingRight) {
            if(SwipeListViewWrap.DEBUG){
                Log.d(SwipeListViewWrap.TAG, "change to right");
            }
        ...
        if (posX < 0 && swipingRight) {
            if(SwipeListViewWrap.DEBUG){
                Log.d(SwipeListViewWrap.TAG, "change to left");
            }
            swipingRight = !swipingRight;
        ...
        } else if (swipeCurrentAction == SwipeListViewWrap.SWIPE_ACTION_CHOICE) {
    ...
    /**
     * Class that saves pending dismiss initData
     */
